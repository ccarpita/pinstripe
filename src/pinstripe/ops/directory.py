...
class Directory(Entity):
    def __init__(self, context: Context, path: str, label=""):
        name = name or f"Drectory: {path}"
    ...
    def execute(self) -> Result:
        ...
        if self.state == "absent":
            res = self.execute_command(["rm", "-rf", self._path])
            res.changed = True
            return res
        ...
        if not stat.ok:
            mkdirp = self.execute_command(["mkdir", "-p", self._path])
            changed = True
            if not mkdirp.ok:
                return mkdirp
